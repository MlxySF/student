...
require_once 'send_payment_approval_email.php'; // ✅ ADDED: Email function
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    // ✅ VERIFY PAYMENT - WITH EMAIL NOTIFICATION
    if ($_POST['action'] === 'verify_payment') {
        ...
        $stmt = $pdo->prepare("
            UPDATE payments 
            SET verification_status = ?, admin_notes = ?, verified_by = ?, verified_date = NOW()
            WHERE id = ?
        ");
        ...
        // If verified, update invoice status to paid AND send email
        if ($verification_status === 'verified') {
            ...
            try {
                error_log("[admin.php] Calling sendPaymentApprovalEmail...");
                $emailSent = sendPaymentApprovalEmail($payment_id, 'verified', $admin_notes);
                error_log("[admin.php] Email sent result: " . ($emailSent ? 'SUCCESS' : 'FAILED'));
            } catch (Exception $e) {
            ...
            if ($emailSent) {
                $message .= " Email with receipt sent to parent.";
            } else {
            ...
            try {
                error_log("[admin.php] Calling sendPaymentApprovalEmail for rejection...");
                $emailSent = sendPaymentApprovalEmail($payment_id, 'rejected', $admin_notes);
                error_log("[admin.php] Rejection email sent: " . ($emailSent ? 'SUCCESS' : 'FAILED'));
            } catch (Exception $e) {
                error_log("[admin.php] Rejection email error: " . $e->getMessage());
            }
