...
$sql = "SELECT id, registration_number, name_en, name_cn, ic, age, school, student_status, phone, email, level, events, schedule, parent_name, parent_ic, form_date, signature_base64, pdf_base64, payment_amount, payment_date, payment_receipt_base64, payment_status, class_count, student_account_id, account_created, password_generated, parent_account_id, registration_type, is_additional_child, created_at FROM registrations";
...
<div class="card">
    ...
    <div class="card-body">
        <div class="mb-4">
            ...
            <button id="bulkSelectBtn-registrations" class="btn btn-primary ms-2"><i class="fas fa-check-square"></i> Select</button>
        </div>
        ...
        <div id="bulkActions-registrations" class="bulk-actions">
            <div class="d-flex align-items-center gap-3">
                ...
                <button id="bulkDeleteBtn-registrations" class="btn btn-bulk-delete" disabled><i class="fas fa-trash-alt"></i> Delete Selected</button>
            </div>
        </div>
            ...
            <table class="table table-striped data-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?php $statusValue = $reg['payment_status'] ?? ''; $badgeColor = $statusValue === 'approved' ? 'success' : ($statusValue === 'pending' ? 'warning' : ($statusValue === 'rejected' ? 'danger' : 'secondary')); $statusText = $statusValue === 'approved' ? 'Approved' : ($statusValue === 'pending' ? 'Pending' : ($statusValue === 'rejected' ? 'Rejected' : 'No Status')); ?><span class="badge bg-<?php echo $badgeColor; ?>"><?php echo htmlspecialchars($statusText); ?></span></td>
                        <td><?php echo date('M j, Y', strtotime($reg['created_at'])); ?></td>
                        <td><button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#viewModal<?php echo $reg['id']; ?>"><i class="fas fa-eye"></i></button></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
