...
if ($action === 'create_invoice') {
    ...
    // ✨ NEW: Get invoice ID and send notification email
    $invoiceId = $pdo->lastInsertId();
    $emailSent = false;
    try {
        $emailSent = sendInvoiceNotification($pdo, $invoiceId);
    } catch (Exception $e) {
        error_log("[Create Invoice] Email notification failed: " . $e->getMessage());
    }
    
    $message = "Invoice created! #$invoice_number";
    if ($emailSent) {
...
if ($action === 'generate_monthly_invoices') {
    try {
        ...
        $created = 0;
        $skipped = 0;
        $emailsSent = 0;        // ✨ NEW: Track email success
        $emailsFailed = 0;      // ✨ NEW: Track email failures
        ...
        foreach ($enrollments as $enroll) {
            ...
            // ✨ NEW: Get the invoice ID that was just created
            $invoiceId = $pdo->lastInsertId();
            $created++;
        ...
        // ✨ NEW: Enhanced success message with email stats
