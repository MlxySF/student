 ...
 * UPDATED: Generate professional signed agreement PDF using new template matching old design
 */
...
require_once 'generate_signed_agreement_pdf.php';
...
// =========================
// NEW: Generate professional signed agreement PDF
// UPDATED: Creates PDF using new template matching old design
// =========================
function generateSignedAgreementPDF(array \$params): string {
    try {
        \$pdfGenerator = new SignedAgreementPDF(\$params);
        \$pdfGenerator->generatePDF();
        \$pdfOutput = \$pdfGenerator->Output('S'); // Return as string
        \$pdfBase64 = base64_encode(\$pdfOutput);
        
        error_log("[PDF Generation] Successfully generated signed agreement PDF for {\$params['student_name']}");
        return \$pdfBase64;
...
try {
    ...
    if (\$duplicateCheck) {
        ...
        if (\$status === 'approved') {
            ...
            throw new Exception(
                "A student with the name '{\$childNameEn}' is already registered and APPROVED (Registration #: {\$duplicateCheck['registration_number']}). " .
                "If this is a different student, please use a slightly different English name (e.g., add middle name or initial). " .
                "Contact admin at {\$duplicateCheck['email']} if you need assistance."
            );
        } elseif (\$status === 'pending') {
            // Don't allow registration - pending account exists
            throw new Exception(
                "A student with the name '{\$childNameEn}' already has a PENDING registration (Registration #: {\$duplicateCheck['registration_number']}). " .
                "Please wait for admin approval or contact the academy for assistance. Registration date: " . date('M j, Y', strtotime(\$duplicateCheck['created_at']))
            );
    ...
    // ==========================
    // NEW: Generate professional signed agreement PDF
    // UPDATED: Uses new PDF template instead of plain base64
    // ==========================
    \$pdfParams = [
    ...
    \$generatedPdfBase64 = generateSignedAgreementPDF(\$pdfParams);
    ...
    \$stmt->execute([
        ...
        \$generatedPdfBase64, // Use generated professional PDF
        \$paymentAmount,
